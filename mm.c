...
int mm_init(void)
{
    ...
    PUT(heap_listp + (3 * WSIZE), PACK(16, 0));    // 블럭의 헤더
    PUT(heap_listp + (4 * WSIZE), NULL);           // 블럭에서 P를 가리킬 위치
    PUT(heap_listp + (5 * WSIZE), NULL);           // 블럭에서 N을 가리킬 위치
    PUT(heap_listp + (6 * WSIZE), PACK(16, 0));    // 블럭 풋터
    PUT(heap_listp + (7 * WSIZE), PACK(0, 1));     // 에필로그 풋터
...
static void *find_fit(size_t asize) // best fit
{
    ...
    // for (bp = free_listp; GET_ALLOC(HDRP(bp)) == 0; bp = NEXT_FLP(bp))
    for (bp = free_listp; bp != NULL; bp = NEXT_FLP(bp))
        if (asize <= (size_t)GET_SIZE(HDRP(bp)))
